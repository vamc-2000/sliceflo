...
import { RelationshipDetailDialog } from "@/components/projects/views/list-view/common/RelationshipDetailDialog";
...
export function TeamWorkTaskTable({
    ...
    const getTaskConfigs = React.useCallback((pId: string) => {
        const p = projects.find((proj) => proj.id === pId);
        return {
            slug: p?.slug ?? "TASK",
            statusConfigs: p?.taskStatusConfig || [],
            priorityConfigs: p?.taskPriorityConfig || [],
            types: p?.taskTypeConfig || [],
            project: p,
        };
    }, [projects]);

    const getActiveOrUpcomingCycles = React.useCallback((projObj: any) => {
        if (!projObj?.cycles) return [];
        const now = new Date();
        return projObj.cycles.filter((c: any) => {
            const start = new Date(c.startDate);
            const end = new Date(c.endDate);
            const isActive = isWithinInterval(now, { start, end });
            const isUpcoming = isFuture(start);
    ...
    const customFields = isTeamView ? [] : (projectId ? getTaskCustomFields(projectId) : []);
    const taskTypes = isTeamView ? [] : (projectId ? getTaskTypesByProject(projectId) : []);
    const taskStatusConfigs = isTeamView ? [] : (projectId ? getTaskStatusConfigs(projectId) : []);
    const taskPriorityConfigs = isTeamView ? [] : (projectId ? getTaskPriorityConfigs(projectId) : []);
    ...
    const resolveProjectId = React.useCallback((tId?: string, isSub?: boolean, subId?: string) => {
        if (isTeamView) {
            if (subId) {
                const sub = storeSubtasks.find(s => s.id === subId);
                if (sub?.projectId) return sub.projectId;
            }
            if (tId) {
                const t = tasks.find(x => x.id === tId);
                if (t?.projectId) return t.projectId;
            }
            return newTaskData.projectId || "";
        }
        return projectId || "";
    }, [isTeamView, storeSubtasks, tasks, newTaskData.projectId, projectId]);
    ...
    const handleSaveTask = async () => {
        ...
        const targetProjectId = isTeamView ? newTaskData.projectId : projectId;
        if (!targetProjectId) {
        ...
        if (
            ...
            const config = (isTeamView ? getTaskConfigs(targetProjectId).statusConfigs : taskStatusConfigs).find((c: any) => c.label === groupName);
            status = config?.value;
        }
        const allowedStatuses = (isTeamView ? getTaskConfigs(targetProjectId).statusConfigs : taskStatusConfigs).map((c: any) => c.value);
        if (status && !allowedStatuses.includes(status)) {
            status = (isTeamView ? getTaskConfigs(targetProjectId).statusConfigs : taskStatusConfigs)[0]?.value || "backlog";
        }
        ...
        if (groupBy === "taskType" && groupName && groupName !== "Untitled") {
            const type = (isTeamView ? getTaskConfigs(targetProjectId).types : taskTypes).find((t: any) => t.label === groupName);
        ...
        if (
            ...
            const cycle = (isTeamView ? getTaskConfigs(targetProjectId).project : project)?.cycles?.find((c: any) => c.name === groupName);
    ...
    const handleSaveSubtask = async (parentTaskId: string) => {
        ...
        const targetProjectId = isTeamView ? (parentTask?.projectId || "") : (projectId || "");
        ...
        const allowedStatuses = (isTeamView ? getTaskConfigs(targetProjectId).statusConfigs : taskStatusConfigs).map((c: any) => c.value);
        if (status && !allowedStatuses.includes(status)) {
            status = parentTask?.status || (isTeamView ? getTaskConfigs(targetProjectId).statusConfigs : taskStatusConfigs)[0]?.value || "backlog";
        }
        ...
        if (cycleId === undefined && parentTask?.cycleId) {
    ...
    const handleAddStatus = async (
        ...
        if (!name.trim()) return;
        const targetProjectId = resolveProjectId(taskId, isSubtask, subtaskId);
        if (!targetProjectId) {
            toast("error", { title: "Cannot determine project to add status to" });
            return;
        }
        await addTaskStatusConfig(targetProjectId, {
            label: name,
            color: "#6B7280",
    ...
    const handleAddPriority = async (
        ...
        const targetProjectId = resolveProjectId(taskId, isSubtask, subtaskId);
        if (!targetProjectId) {
            toast("error", { title: "Cannot determine project to add priority to" });
            return;
        }
        const value = name.trim().toLowerCase().replace(/\s+/g, "_");
        const configs = isTeamView ? getTaskConfigs(targetProjectId).priorityConfigs : taskPriorityConfigs;
        await addTaskPriorityConfig(targetProjectId, {
            label: name.trim(),
            value,
            description: "",
            color: "#6B7280",
    ...
    const getColumnStyle = (
        ...
        const alwaysFrozenColumns: Record<string, { width: number; order: number }> = {
        ...
        if (isTeamView) {
            alwaysFrozenColumns.project = { width: 150, order: 2 };
        }
        if (alwaysFrozenColumns[columnId as keyof typeof alwaysFrozenColumns]) {
            ...
            if (columnId === "project")
                leftOffset =
                    ...
                    CHECKBOX_COL_WIDTH +
                    ID_COL_WIDTH +
                    Math.max(columnWidths["task"] ?? TASK_COL_DEFAULT_WIDTH, 150);
    ...
    const getPriorityColor = (
        ...
        const configs = pId
            ? getTaskConfigs(pId).priorityConfigs
            : taskPriorityConfigs;
    ...
    const taskCheckboxClass =
        "size-4 shrink-0 rounded flex items-center justify-center [&_[data-slot=checkbox-indicator]]:flex [&_[data-slot=checkbox-indicator]]:items-center [&_[data-slot=checkbox-indicator]]:justify-center [&_[data-slot=checkbox-indicator]_svg]:size-2.5";
                                ...
                                return (
                                    <React.Fragment key={task.id}>
                                        ...
                                        <DraggableTaskRow
                                            ...
                                            projectId={isTeamView ? (task.projectId || "") : (projectId || "")}
                                            projectSlug={isTeamView ? getTaskConfigs(task.projectId).slug : projectSlug}
                                            ...
                                            taskTypes={isTeamView ? getTaskConfigs(task.projectId).types : taskTypes}
                                            renderTaskTypeVisual={renderTaskTypeVisual}
                                            updateTask={updateTask}
                                            taskStatusConfigs={isTeamView ? getTaskConfigs(task.projectId).statusConfigs : taskStatusConfigs}
                                            ...
                                            activeOrUpcomingCycles={isTeamView ? getActiveOrUpcomingCycles(getTaskConfigs(task.projectId).project) : activeOrUpcomingCycles}
                                            formatCycleName={formatCycleName}
                                            project={isTeamView ? getTaskConfigs(task.projectId).project : project}
                                            members={members}
                                            ...
                                            taskPriorityConfigs={isTeamView ? getTaskConfigs(task.projectId).priorityConfigs : taskPriorityConfigs}
                                        ...
                                        {isExpanded &&
                                            taskSubtasks.map((subtask) => {
                                                const subtaskProjectId = subtask.projectId || task.projectId || "";
                                                const subtaskConfigs = getTaskConfigs(subtaskProjectId);
                                                const currentTaskTypes = isTeamView ? subtaskConfigs.types : taskTypes;
                                                const currentStatusConfigs = isTeamView ? subtaskConfigs.statusConfigs : taskStatusConfigs;
                                                const subtaskProject = isTeamView ? subtaskConfigs.project : project;
                                                const currentCycles = isTeamView ? getActiveOrUpcomingCycles(subtaskProject) : activeOrUpcomingCycles;
                                                const currentPriorityConfigs = isTeamView ? subtaskConfigs.priorityConfigs : taskPriorityConfigs;
                                                const currentPriorityColor = getPriorityColor(subtask.priority, subtaskProjectId);
                                                return (
                                                    <TableRow
                                                        key={subtask.id}
                                                                ...
                                                                <DropdownMenu>
                                                                    ...
                                                                    >
                                                                        <button className="w-full h-full flex items-center justify-center cursor-pointer hover:bg-muted transition-colors overflow-hidden px-3 gap-2">
                                                                            {(() => {
                                                                                const subtaskProjectId = subtask.projectId || task.projectId || "";
                                                                                const currentTaskTypes = isTeamView ? getTaskConfigs(subtaskProjectId).types : taskTypes;
                                                                                const selectedType =
                                                                                    currentTaskTypes.find(
                                                                                        (t) =>
                                                                                            t.value ===
                                                        ...
                                                        {shouldShowField("cycle", "Cycle") && (
                                                            ...
                                                            >
                                                                <DropdownMenu>
                                                                        ...
                                                                        <button className="w-full h-full flex items-center justify-center rounded-xs text-foreground text-xs font-medium transition-opacity hover:bg-muted overflow-hidden px-3">
                                                                            <span className="truncate w-full text-center flex items-center justify-center">
                                                                                {subtask.cycle
                                                                                    ...
                                                                                    : (() => {
                                                                                        const c = subtaskProject?.cycles?.find(
                                                                                            (c: any) => c.id === subtask.cycleId,
                                                                                        );
                                                                                        return c ? (
                                                                                            formatCycleName(
                                                                                                c.name,
                                                                    ...
                                                                    <DropdownMenuContent className="p-4 w-[200px] space-y-1">
                                                                        {(currentCycles as import("@/lib/api/projects-api").Cycle[]).map((c) => (
                                                                            <DropdownMenuItem
                                                                                key={c.id}
                                                                                onSelect={() =>
                                                                                    updateSubtask(subtask.id, {
            ...
            <DialogFooter>
              ...
              >
                ...
                <Dialog open={relDialogOpen} onOpenChange={setRelDialogOpen}>
                    <DialogContent className="sm:max-w-md border-b-[5px] border-b-primary">
                        ...
                        <div className="space-y-4 py-2">
                            ...
                            {relDialogTaskId &&
                                (() => {
                                    const targetProjectId = resolveProjectId(relDialogTaskId ?? undefined, relDialogIsSubtask) || projectId || "";
                                    const projectTasks = getTasksByProject(targetProjectId);
                                    const allTasks = [
                                        ...projectTasks,
                ...
                <TaskDetailView
                    ...
                    projectId={selectedTaskForDetail?.projectId || projectId || ""}
                    open={showTaskDetail}
                    onOpenChange={(open) => {
                ...
                <TaskDetailView
                    ...
                    projectId={selectedSubtaskForDetail?.projectId || projectId || ""}
                ...
                {convertToSubtaskTaskId &&
                    (() => {
                        const taskToConvert = groupTasks.find((t) => t.id === convertToSubtaskTaskId) ?? null;
                        const targetProjectId = taskToConvert?.projectId || projectId || "";
                        const targetProject = projects.find(p => p.id === targetProjectId);
                        const targetPriorityConfigs = targetProjectId ? getTaskConfigs(targetProjectId).priorityConfigs : taskPriorityConfigs;
                        return (
                            <ConvertToSubtaskDialog
                                open={!!convertToSubtaskTaskId}
                                onClose={() => setConvertToSubtaskTaskId(null)}
                                taskToConvert={taskToConvert}
                                availableTasks={getTasksByProject(targetProjectId)}
                                members={members}
                                priorityConfigs={targetPriorityConfigs}
