...
interface ProjectMembersSectionProps {
  ...
  members: Array<{
    ...
    addedViaTeam?: { id: string; name: string };
  }>;
  onAddMembers: (
    members: Array<{
      userId: string;
...
const ProjectMembersSection: React.FC<ProjectMembersSectionProps> = ({
  ...
  const { addMembersToProjectViaTeam, addUserToProjectFromTeam } =
    useProjectsStore();
    ...
    return members
      .map((member) => {
        ...
        return {
          ...
          teamName: teamName,
          fullProfilePictureUrl: getProfilePictureUrl(
            workspaceMember.profilePicture,
          ),
          initials:
            ...
            <div className="space-y-0.5 max-h-80 overflow-y-auto pr-1">
              {filteredTeams.length === 0 ? (
                ...
                filteredTeams.map((team) => {
                  ...
                  const isSelected =
                    !isTeamFullyAdded && selectedTeams.has(team.id);
                              ...
                              {team.teamMembers?.length > 0 ? (
                                <>
                                  ...
                                  <Button
                                    ...
                                    disabled={
                                      (selectedTeamMembers[team.id]?.size ??
                                        0) === 0 ||
                                      isLoading ||
                                      addedTeams.has(team.id)
                                    }
                                    onClick={async () => {
                                      ...
                                      if (newMembers.length === 0) {
                                        toast("warning", {
                                          title:
                                            "All selected members are already in this project",
                                        });
                                        return;
                                      }
                                      ...
                                      setIsLoading(true);
                                      try {
                                        await addUserToProjectFromTeam(
                                          projectId,
                                          newMembers,
                                        );
                                        ...
                                        toast("success", {
                                          title: `${newMembers.length} member${newMembers.length > 1 ? "s" : ""} added from ${team.name}`,
                                        });
                                      } catch {
                                        toast("error", {
          ...
          filteredMembers.map((member) => (
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  {/* Member name */}
                  <p className="text-xs truncate text-primary-text">
                    {member?.name}
                  </p>
