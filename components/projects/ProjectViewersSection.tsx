...
const ProjectViewersSection: React.FC<ProjectViewersSectionProps> = ({
  ...
  const { workspaceMembers, currentWorkspace, fetchWorkspaceMembers } =
    useWorkspaceStore();
  ...
  const handleAddTeam = async (teamId: string) => {
    ...
    try {
      ...
      toast("success", {
        title: `Added ${newViewerIds.length} team member${newViewerIds.length > 1 ? "s" : ""} as viewers`,
      });
      setSearchQuery(""); // Clear search after adding
    } catch (error) {
  ...
  const toggleTeamMemberSelect = (
    ...
    setSelectedTeamMembers((prev) => {
      ...
      const allSelected =
        allIds.length > 0 && allIds.every((id) => teamSet.has(id));
        ...
        >
          <div className="flex items-center">
            ...
            <TabsList className="w-full flex gap-1 bg-transparent border-b rounded-none p-0 justify-end">
              <TabsTrigger
                value="teams"
                className="rounded-none border-0 border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary-text text-xs"
              ...
              {filteredTeams.length === 0 ? (
                ...
                filteredTeams.map((team) => {
                  ...
                  return (
                    <div
                      key={team.id}
                      className="rounded-md border border-transparent hover:border-accent"
                    >
                      {/* TEAM ROW */}
                      <div
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <button
                            ...
                            data-testid={`project-viewers-team-expand-btn-${team.id}`}
                          >
                            <ChevronRight
                          ...
                          <Avatar className="h-7 w-7 shrink-0">
                            {(() => {
                              ...
                              if (avatar?.type === "icon") {
                                const iconObj = iconLibrary.find(
                                  (i) => i.name === avatar.name,
                                );
                                if (iconObj) {
                            ...
                            <>
                              {team.teamMembers.map((member: any) => (
                                ...
                                >
                                  ...
                                  <Checkbox
                                    checked={selectedTeamMembers[team.id]?.has(
                                      member.id,
                                    )}
                                    disabled={
                                      isLoading || addedTeams.has(team.id)
                                    }
                                    onCheckedChange={(checked) =>
                                      toggleTeamMemberSelect(
                                        team.id,
                                        member.id,
                                        Boolean(checked),
                                      )
                              ...
                              <Button
                                ...
                                disabled={
                                  (selectedTeamMembers[team.id]?.size ?? 0) ===
                                    0 ||
                                  isLoading ||
                                  addedTeams.has(team.id)
                                }
                                onClick={async () => {
                                  console.log(
                                    "Selected members for team",
                                    team.id,
                                    selectedTeamMembers[team.id],
                                  );
                                  const selectedIds = Array.from(
                                    selectedTeamMembersRef.current[team.id] ??
                                      [],
                                  );
                                  if (selectedIds.length === 0) {
                                    console.log(
                                      "No members selected for team",
                                      team.id,
                                    );
                                    toast("error", {
                                  ...
                                  const newViewerIds = selectedIds.filter(
                                    (id) => !viewers.includes(id),
                                  );
                                  console.log(
                                    "New viewer IDs to add for team",
                                    team.id,
                                    newViewerIds,
                                  );
                                  if (newViewerIds.length === 0) {
                                    console.log(
                                      "All selected members are already viewers for team",
                                      team.id,
                                    );
                                    toast("info", {
                                      title:
                                        "All selected members are already viewers",
                                    });
                                    return;
                                  }
                                  ...
                                  try {
                                    console.log(
                                      "Adding team members as viewers:",
                                      newViewerIds,
                                    );
                                    await onAddViewers(newViewerIds);
                                    setAddedTeams((prev) =>
                                      new Set(prev).add(team.id),
                                    );
                                    setSelectedTeamMembers((prev) => {
                                      const next = { ...prev };
                                      delete next[team.id];
                                    ...
                                    toast("success", {
                                      title: `Team "${team.name}" added as viewers`,
                                    });
                                  } catch {
                                    toast("error", {
                ...
                filteredAvailableMembers.map((member) => (
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <Avatar className="h-7 w-7 shrink-0">
                        <AvatarImage
                          src={member.fullProfilePictureUrl}
                          alt={member.name}
                        />
    ...
    <div className="space-y-2">
        ...
        {viewerDetails.length > 0 && (
          <div className="flex items-center -space-x-2">
            {displayedViewers.map((viewer, index) => (
              ...
              >
                <AvatarImage
                  src={viewer?.fullProfilePictureUrl}
                  alt={viewer?.name}
                />
      ...
      <div className="grid grid-cols-2 gap-2">
        <Button
          ...
          onClick={onInviteClick}
          className="bg-primary hover:bg-primary/90 text-primary-foreground h-8 text-xs"
          data-testid="project-viewers-invite-btn"
        >
          <Plus className="h-4 w-4 mr-1" /> Invite
        </Button>
        <Button
          ...
          className="bg-primary hover:bg-primary/90 text-primary-foreground h-8 text-xs"
          data-testid="project-viewers-add-btn"
        >
