...
export default function TransferCycleTasksDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      const sourceSubtasks = subtasks.filter(
        (st) =>
      ...
      if (!addedIds.has(child.id)) {
        ...
        if (matchesQuery(child)) {
          if (
            ...
            (parent.cycleId === sourceCycleId ||
              parent.cycle?.id === sourceCycleId)
          ) {
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      selectedIds.forEach((taskId) => {
        ...
        const originalCycleId =
          originalTask?.cycleId || originalTask?.cycle?.id || "";
        ...
        if (targetCycleId !== originalCycleId) {
          if (targetCycleId) {
            ...
            removePromises.push(
              removeTasksFromCycle(projectId, originalCycleId, [taskId]),
            );
          }
        }
      });
      ...
      Object.entries(tasksByCycle).forEach(([targetCycleId, taskIds]) => {
        assignPromises.push(
          assignTasksToCycle(projectId, targetCycleId, taskIds),
        );
      });
      ...
      <DialogContent className="max-w-xl w-full border-b-[5px] border-b-primary">
        ...
        <div className="space-y-3 w-full">
          ...
          <div className="flex gap-2 items-center">
            ...
            {selectedIds.size > 0 && (
              <div className="flex items-center gap-1.5 bg-primary/5 border border-primary/20 px-2.5 py-1 rounded-md h-9">
                <span className="text-[10px] font-bold text-primary-text whitespace-nowrap">
                  Set selected ({selectedIds.size}) to:
                </span>
                <select
                  onChange={(e) => handleBulkCycleChange(e.target.value)}
                  className="bg-background border border-primary/20 rounded px-1.5 py-0.5 text-[10px] font-bold text-primary-text focus:outline-none focus:ring-1 focus:ring-primary"
                  defaultValue=""
                >
          ...
          <div className="w-full border border-border rounded-md relative">
            {/* Table Header */}
            <div className="grid grid-cols-[45px_70px_1fr_100px_100px_40px] px-3 py-2 text-xs font-semibold items-center text-primary-text border-b bg-muted/40">
              <div className="flex items-center">
                <Checkbox
                  checked={
                    availableTasks.length > 0 &&
                    selectedIds.size === availableTasks.length
                  }
                    ...
                    >
                      ...
                      <div className="pr-1.5">
                        <select
                          ...
                          onChange={(e) =>
                            handleCycleChange(task.id, e.target.value)
                          }
                        ...
                        >
                          <option value="">No Cycle</option>
                          {sourceCycle &&
                            taskCycles[task.id] === sourceCycleId && (
