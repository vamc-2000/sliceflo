...
export function CycleProgressCard({
  ...
  if (isEmpty) {
    return (
      <div className="flex flex-col space-y-2">
        ...
        <div
          data-testid="cycle-progress-card-empty"
          className="bg-primary/10 rounded-md p-2 flex flex-col items-center justify-center text-center shadow-sm min-h-[280px] border-b-4 border-primary/50 relative overflow-hidden"
        >
          <div className="flex-1 flex flex-col items-center justify-center mt-4">
            ...
            <p className="text-primary-text text-xs max-w-[200px] leading-relaxed">
  ...
  const statusChartData = statusConfig
    .map((status) => {
  ...
  const priorityChartData = priorityConfig
    .map((priority) => {
      const count = tasks.filter(
        (t) => t.priority === priority.value || t.priority === priority.label,
      ).length;
      return {
        name: priority.label,
        value: count,
  ...
  const chartData =
    activeTab === "status" ? statusChartData : priorityChartData;
  const currentLegendConfig =
    activeTab === "status" ? statusConfig : priorityConfig;
  ...
  return (
    <div className="flex flex-col space-y-2">
      <h3 className="text-sm font-semibold text-foreground">Progress</h3>
      <div
        data-testid="cycle-progress-card-chart"
        className="bg-card border border-border rounded-md p-3 shadow-sm min-h-[280px] flex flex-col justify-between"
      >
        ...
        <div className="flex-1 flex flex-row items-center justify-between px-2 pt-1">
          ...
          <div className="relative w-36 h-36 flex-none">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span
                data-testid="cycle-progress-card-total-tasks"
            ...
            {currentLegendConfig.map((item) => {
              const count =
                activeTab === "status"
                  ...
                  : tasks.filter(
                      (t) =>
                        t.priority === item.value || t.priority === item.label,
                    ).length;
              return (
                <div
                  data-testid={`cycle-progress-card-legend-${item.value}`}
                  key={item.value}
