    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          className="gap-1 h-8 rounded text-xs text-muted-foreground bg-background hover:bg-muted hover:text-foreground"
          data-testid="custom-views-more-trigger"
        >
          More
      ...
      <DropdownMenuContent
        align="end"
        className="w-60 p-2 border-b-[5px] border-b-primary bg-popover"
      >
        <Tabs defaultValue="popular" className="w-full">
          ...
          <TabsList className="w-full rounded p-0">
            <TabsTrigger
              value="popular"
              className="rounded text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground w-full"
              ...
              {POPULAR_VIEWS.map((view) => {
                ...
                return (
                  <Button
                    ...
                    key={view.id}
                    onClick={() =>
                      handleViewClick(view.id, view.name, view.iconName)
                    }
                    disabled={isSelected}
                    className="flex justify-start items-center gap-2 p-1.5 transition-colors text-left h-auto w-full"
