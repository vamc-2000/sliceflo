...
export function TaskDetailView({
  ...
  useEffect(() => {
    if (open && !isSubtask && task?.taskType === "milestone") {
            ...
            .map((doc) => {
              ...
              const renderTree = (d: any, level = 0) => {
                const isLinkedToTask = (
                  currentTask.linkedDocuments || []
                ).includes(d.id);
                return (
                  <div key={d.id} className="space-y-1">
                    <div
                      className="flex items-center gap-2 py-1 px-2 rounded hover:bg-muted group cursor-pointer"
                      onClick={() => {
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          className={cn(
                            "text-xs truncate",
                            isLinkedToTask && "text-muted-foreground",
                          )}
                        >
                          {d.title}
                        </span>
                      ...
                      <Checkbox
                        checked={
                          isLinkedToTask || selectedDocsForTask.has(d.id)
                        }
                        ...
                        className={cn(
                          isLinkedToTask && "opacity-50 cursor-not-allowed",
                        )}
                      />
                    </div>
                    {isExpanded &&
          ...
          <VisuallyHidden asChild>
            <div>
              <DialogPrimitive.Title>
                {isMilestone ? "Milestone Details" : "Task Details"}
              </DialogPrimitive.Title>

              <DialogPrimitive.Description>
                {isMilestone
                  ? "View and manage milestone details including description, subtasks, checklist, relationships and properties."
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center justify-between p-4">
                                <Link
                                  href={`/docs/${doc.id}`}
                                  className="flex items-center gap-4 min-w-0 flex-1 group/doc hover:opacity-80 transition-opacity"
                                >
                      ...
                      {relationships.length > 0 && (
                        <div className="space-y-2">
                          {relationships.map((rel) => {
                            const targetTask = projectTasks.find(
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  {(() => {
                                    const RelIcon = getRelationshipIcon(
                                      rel.type,
                                    );
                                    return (
                                      <RelIcon
