...
const DuplicateTaskDialog: React.FC<DuplicateTaskDialogProps> = ({
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      const fields = mode === "system" ? SYSTEM_FIELDS : selectedIds;
  ...
  const renderCheckbox = (field: (typeof TASK_DUPLICATE_FIELDS)[number]) => {
    ...
    const disabled =
      "dependsOn" in field && !selectedIds.includes(field.dependsOn as FieldId);
    return (
      <label
        key={field.id}
        className={cn(
      ...
      <DialogContent className="sm:max-w-[480px] border-b-[5px] border-b-primary p-0 overflow-hidden">
              ...
              <div className="rounded-lg border p-1.5">
                ...
                {mode === "system" && (
                  <p className="mt-2 px-1 text-xs text-muted-foreground">
                    All fields including custom fields and settings will be
                    duplicated exactly as is.
                  </p>
                )}
                ...
                {mode === "customize" && (
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 px-1">
                      <div className="space-y-2">
                        {leftFields.map(renderCheckbox)}
                      </div>
                      <div className="space-y-2">
                        {rightFields.map(renderCheckbox)}
                      </div>
                    </div>
                  </div>
                )}
        ...
        <DialogFooter className="flex items-center justify-between px-6 py-3">
          <Button
            ...
            onClick={onClose}
            disabled={loading}
            data-testid="duplicate-task-cancel-btn"
          >
            Cancel
          </Button>
          <Button
