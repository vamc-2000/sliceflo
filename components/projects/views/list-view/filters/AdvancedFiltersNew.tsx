...
export default function AdvancedFiltersNew({
  ...
  const getValueOptionsForField = (
    ...
    if (fieldId === "priority") {
      return taskPriorityConfigs.map((c) => ({
        value: c.value,
        label: c.label,
      }));
    }
  ...
  const isValid = () => {
    return filterBlock.children.some((criteria) => {
      const isEmptyCondition =
        criteria.condition === "is-empty" ||
        criteria.condition === "is-not-empty";
      const isSpecialDateCondition = [
        "date-is-today",
        "date-is-this-week",
        "date-is-this-month",
      ].includes(criteria.condition);
      return (
        criteria.field &&
        criteria.condition &&
        (isEmptyCondition || isSpecialDateCondition || criteria.value)
      );
    });
  };
  ...
  const renderValueInput = (criteria: FilterCriteria) => {
    ...
    if (
      ...
      return (
        <div className="px-3 py-2 text-xs text-foreground">{displayText}</div>
      );
    }
        ...
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {criteria.value ? (
                format(new Date(criteria.value), "PPP")
              ) : (
          ...
          >
            <CalendarPicker
              selectedDate={
                criteria.value
                  ? convertUTCToCalendarDate(criteria.value)
                  : undefined
              }
              onDateSelect={(date) =>
          ...
          <TabsContent value="advanced" className="pt-4 space-y-4">
            ...
            {filterBlock.children.map((criteria, index) => (
              ...
              >
                ...
                {/* Value Input */}
                <div className="flex-1">{renderValueInput(criteria)}</div>
