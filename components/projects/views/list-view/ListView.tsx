...
import AdvancedFiltersNew, {
  FilterBlock,
  FilterCriteria,
} from "./filters/AdvancedFiltersNew";
...
export function ListView({
  ...
  const members = projectMembers.map(({ userId }) => {
    ...
    return {
      id: userId,
      name: wm?.name || userId,
      avatar: wm?.avatar || wm?.profilePicture || "",
    };
  });
  ...
  const [emptyGroupExceptions, setEmptyGroupExceptions] = useState<Set<string>>(
    new Set(),
  );
  ...
  const [selectedTasksByGroup, setSelectedTasksByGroup] = useState<
  ...
  const [bulkConvertSubtaskQueue, setBulkConvertSubtaskQueue] = useState<
    string[]
  >([]);
  ...
  const handleBulkConvertToSubtask = () => {
    ...
    if (eligible.length === 0) {
      toast("error", {
        title:
          "None of the selected tasks can be converted (tasks with subtasks are excluded)",
      });
      return;
    }
  ...
  const handleBulkConvertSubtaskConfirm = async (
    ...
    toast("success", {
      title: `Converted task to subtask (${bulkConvertSubtaskIndex + 1}/${bulkConvertSubtaskQueue.length})`,
    });
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (newGroupInputRef.current) {
            newGroupInputRef.current.scrollIntoView({
              behavior: "smooth",
              block: "nearest",
            });
  ...
  const dynamicColumns = sortFields.filter(
    (f) =>
      ![
        "id",
        "task",
  ...
  const handleFreezeToggle = (fieldId: string) => {
    ...
    const sortedFields = [...columnConfigs].sort(
    ...
    return dynamicGroups
      .filter((group: DynamicGroup) => {
        ...
        if (groupBy === "status") {
          ...
          count = cfg
            ? projectTasks.filter((t) => t.status === cfg.value).length
            : 0;
          ...
          if (group.isUntitled) {
            count = projectTasks.filter((t) => !t.priority).length;
          } else {
            const pId = group.id.replace("priority-", "");
            const pCfg = taskPriorityConfigs.find((p) => p._id === pId);
            count = pCfg
              ? projectTasks.filter((t) => t.priority === pCfg.value).length
              : 0;
          }
        } else if (groupBy === "dueDate") {
          const today = new Date();
          today.setHours(0, 0, 0, 0);
          if (group.id === "date-overdue")
            count = projectTasks.filter((t) => {
              if (!t.endDate) return false;
              const d = new Date(t.endDate);
              d.setHours(0, 0, 0, 0);
              return d < today;
            }).length;
          else if (group.id === "date-today")
            count = projectTasks.filter((t) => {
              if (!t.endDate) return false;
              const d = new Date(t.endDate);
              d.setHours(0, 0, 0, 0);
          ...
          else if (group.id === "date-upcoming")
            count = projectTasks.filter((t) => {
              if (!t.endDate) return false;
              const d = new Date(t.endDate);
              d.setHours(0, 0, 0, 0);
          ...
          else if (group.id === "date-no-date")
            count = projectTasks.filter((t) => !t.endDate).length;
        } else if (groupBy?.startsWith("custom-")) {
          const fieldId = groupBy.replace("custom-", "");
          count =
            group.name === "No Value"
              ? projectTasks.filter((t) => !t.customFieldValues?.[fieldId])
                  .length
              : projectTasks.filter(
  ...
  useEffect(() => {
    dynamicGroupsRef.current = dynamicGroups;
  }, [dynamicGroups]);
  useEffect(() => {
    hideEmptyGroupsRef.current = hideEmptyGroups;
  }, [hideEmptyGroups]);

  // Stable callbacks — created once, never cause parent setState during render
  const stableCollapseAll = useCallback(() => {
    setCollapsedGroups(new Set(dynamicGroupsRef.current.map((g) => g.id)));
  }, []);
  const stableExpandAll = useCallback(() => {
    ...
    if (onRegisterCollapseHandlers) {
      onRegisterCollapseHandlers(
        ...
        stableToggleHideEmpty,
        {
          ...
          allCollapsed:
            dynamicGroupsRef.current.length > 0 &&
  ...
  useEffect(() => {
    if (onRegisterExportHandlers) {
      onRegisterExportHandlers(handleExportCSV, handleExportExcel, handlePrint);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tasks.length]);
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `tasks_${projectId}_${new Date().toISOString().split("T")[0]}.csv`,
    );
    link.click();
    ...
    <DndProvider backend={HTML5Backend}>
            ...
            >
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  data-testid="list-groupby-trigger"
                >
                  ...
                  {previouslyUsedGroupBy.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {previouslyUsedGroupBy.map((item, index) => {
                        const optionData = allOptions.find(
                  ...
                  >
                    <DropdownMenuTrigger asChild>
                      <Button
                        ...
                        variant="ghost"
                        className="gap-2 rounded cursor-pointer text-xs"
                        data-testid="list-filter-trigger"
                      >
                              ...
                              taskStatusConfigs.map((cfg) => (
                                ...
                                >
                                  <div
                                    ...
                                    style={{ backgroundColor: cfg.color || "#c4c4c4" }}
                                  >
                  ...
                  >
                    <DropdownMenuTrigger asChild>
                      <Button
                        ...
                        variant="ghost"
                        className="gap-2 rounded cursor-pointer text-xs"
                        data-testid="list-display-trigger"
                      >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  data-testid="list-unhide-column-trigger"
                >
                  <EyeOff className="h-4 w-4" />
                  ...
                  {(() => {
                    const hiddenSet = new Set(
                      hideEmptyGroups
                        ? emptyGroupIds.filter(
                            (id) => !emptyGroupExceptions.has(id),
                          )
                        : [],
                    );
                    ...
                    dynamicGroups.forEach((g) => {
                      ...
                      if (displayOptions.closedTasks && groupBy === "status") {
                        const cfg = taskStatusConfigs.find(
                          (c) => c._id === g.id,
                        );
              ...
              <DropdownMenuContent
                align="end"
                className="w-64 p-3 border-b-[5px] border-b-primary"
              >
                <h3 className="text-sm font-semibold mb-3">Unhide Group</h3>

                {(() => {
                  const manuallyHidden = dynamicGroups.filter((g) =>
                    hiddenGroups.has(g.id),
                  );
                  const emptyHidden = hideEmptyGroups
                    ? dynamicGroups.filter(
                        (g) =>
                  ...
                  const doneHidden =
                    displayOptions.closedTasks && groupBy === "status"
                  ...
                  const uniqueEmptyHidden = emptyHidden.filter(
                    (g) => !doneHiddenIds.has(g.id),
                  );
                  const uniqueManuallyHidden = manuallyHidden.filter(
                    (g) => !doneHiddenIds.has(g.id),
                  );

                  const allHiddenGroups = [
                    ...uniqueManuallyHidden,
                    ...uniqueEmptyHidden,
                    ...doneHidden,
                  ];
                          ...
                          onClick={() => {
                            ...
                            if (!handled) {
                              ...
                              setEmptyGroupExceptions((prev) => {
                                ...
                                const allExcepted = emptyGroupIds.every((id) =>
                                  next.has(id),
                                );
                                if (allExcepted) {
            ...
            .filter((group: DynamicGroup) => {
              ...
              if (groupBy === "status") {
                ...
                count =
                  group.id === "unassigned"
                    ? projectTasks.filter((t) => !t.assignee).length
                    : projectTasks.filter((t) => t.assignee === group.name)
                        .length;
              } else if (groupBy === "priority") {
                if (group.isUntitled) {
                  count = projectTasks.filter((t) => !t.priority).length;
                  ...
                  const pCfg = taskPriorityConfigs.find((p) => p._id === pId);
                  count = pCfg
                    ? projectTasks.filter((t) => t.priority === pCfg.value)
                        .length
                    : 0;
                }
              } else if (groupBy === "dueDate") {
      ...
      {bulkDuplicateQueue.length > 0 &&
        (() => {
          const task = tasks.find(
            (t) => t.id === bulkDuplicateQueue[bulkDuplicateIndex],
          );
          if (!task) return null;
          return (
            <DuplicateTaskDialog
      ...
      {bulkConvertSubtaskQueue.length > 0 && (
        <ConvertToSubtaskDialog
          ...
          taskToConvert={
            tasks.find(
              (t) => t.id === bulkConvertSubtaskQueue[bulkConvertSubtaskIndex],
            ) ?? null
          }
