...
export function FolderCard({
  ...
  const allChecked =
    allFileIds.length > 0 && allFileIds.every((id) => selectedIds.includes(id));
  const someChecked =
    allFileIds.some((id) => selectedIds.includes(id)) && !allChecked;
  ...
  const uniqueUploaders = files.reduce(
    (acc, file) => {
  ...
  return (
    <div className="space-y-2">
      <div className="w-full flex items-center gap-4">
        ...
        <input
          data-testid={`folder-card-checkbox-${taskName}`}
          type="checkbox"
          checked={allChecked}
          ref={(el) => {
        ...
        <div className="w-full flex items-center gap-3 bg-card shadow-sm border border-border rounded-lg px-3 py-2.5 hover:border-primary/30 hover:shadow-md transition-all">
          {/* Folder icon + name */}
          <div
            data-testid={`folder-card-container-${taskName}`}
          ...
          <div className="flex flex-col items-end gap-1 shrink-0">
            <div className="flex items-center">
              ...
              <button
                data-testid={`folder-card-toggle-button-${taskName}`}
                onClick={() => setOpen(!open)}
                className="text-primary-text transition-colors mr-1"
              >
                {open ? (
