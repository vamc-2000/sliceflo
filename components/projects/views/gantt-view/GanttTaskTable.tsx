...
function TableAvatar({
  ...
  if (!name) {
    return (
      <div
        className={`${dim} rounded-full bg-muted border border-dashed border-border flex items-center justify-center text-muted-foreground mx-auto`}
      >
...
export const GanttTaskTable = React.forwardRef<
  HTMLDivElement,
  GanttTaskTableProps
>(
  (
    ...
    const [internalExpandedTasks, setInternalExpandedTasks] = useState<
      Set<string>
    >(new Set());
    const expandedTasks = propsExpandedTasks || internalExpandedTasks;
    const setExpandedTasks = onToggleExpansion
      ? (val: any) => {}
      : setInternalExpandedTasks; // Stub for internal state only if prop not provided
    ...
    const [internalAddingSubtaskToTask, setInternalAddingSubtaskToTask] =
      useState<string | null>(null);
    const addingSubtaskToTask =
      propsAddingSubtaskToTask !== undefined
        ? propsAddingSubtaskToTask
        : internalAddingSubtaskToTask;
    ...
    const showAddTask =
      propsShowAddTask !== undefined ? propsShowAddTask : internalShowAddTask;
    ...
    const [openDatePopoverId, setOpenDatePopoverId] = useState<string | null>(
      null,
    );
    ...
    const shouldShowField = (fieldKey: string, fieldLabel: string): boolean => {
      ...
      if (isSystemField) {
        ...
        // Custom fields: respect gantt toggle, default hidden
        if (systemFieldVisibility[legacyKey] !== undefined)
          return systemFieldVisibility[legacyKey];
        return false;
      }
    ...
    const headers = useMemo(() => {
      ...
      return [...defaultHeaders, ...customHeaders].filter((h) =>
        shouldShowField(h.key, h.label),
      );
    }, [
      customFields,
      projectId,
      ...
      await addTask(
        {
          ...
          startDate: capturedData.startDate
            ...
            : convertSelectedDateToUTC(new Date()),
    ...
    const renderRelationshipIcons = (item: Task) => {
      ...
      return (
        <div className="flex items-center gap-1.5 ml-2 shrink-0">
          {uniqueRels.map((rel) => {
            ...
            const targetTask =
              allTasks.find((t) => t.id === rel.targetTaskId) ||
            ...
            const isHovered =
              hoveredTaskId === item.id && hoveredRelType === rel.type;
    ...
    const renderCellContent = (
      item: Task | any,
      h: { key: string; label: string; isCustom?: boolean },
      isSubtask: boolean,
    ) => {
      const updateFn = isSubtask ? updateSubtask : updateTask;
      const cellCls =
        "flex items-center justify-center gap-2 w-full h-full cursor-pointer hover:bg-muted/50 px-2 rounded transition-colors group/cell text-center";

      // Custom fields — values live on customFieldValues[fieldId], not item[fieldId]
      const customField = customFields.find((f) => f.id === h.key);
      if (customField || h.isCustom) {
        const fieldId = customField?.id ?? h.key;
        const fieldData = getTaskCustomFieldById(projectId, fieldId);
        if (!fieldData) {
          return (
            <div
              className={cn("px-2 text-xs text-muted-foreground text-center")}
            >
              —
            </div>
          );
        }
        const emptyValue = fieldData.type === "select-many" ? [] : "";
        return (
          <CustomFieldDropdown
            field={fieldData}
            value={item.customFieldValues?.[fieldId] ?? emptyValue}
            onUpdate={(value) =>
              isSubtask
                ? updateSubtaskCustomField(item.id, fieldId, value)
                : updateTaskCustomField(item.id, fieldId, value)
            }
            task={item}
          />
        );
      ...
      if (h.key === "status") {
        const statusCfg = taskStatusConfigs.find(
      ...
      if (h.key === "startDate" || h.key === "endDate") {
        const dateValue = item[h.key]
          ? convertUTCToCalendarDate(item[h.key])
          : undefined;
          ...
          >
            <PopoverTrigger asChild>
              <div className={cellCls}>
                {dateValue ? (
                  <span className="text-xs">{formatDate(item[h.key])}</span>
                ) : (
            ...
            >
              <CalendarPicker
                ...
                onDateSelect={(date) => {
                  if (!date) return;
                  const updates: any = {
                    [h.key]: convertSelectedDateToUTC(date),
                  };
                  if (
                    h.key === "startDate" &&
                    item.endDate &&
                ...
                disabled={(date) => {
                  if (h.key === "startDate") {
                    const endDateCal = item.endDate
                      ? convertUTCToCalendarDate(item.endDate)
                      : undefined;
                    return endDateCal ? date > endDateCal : false;
                  } else {
                    const startDateCal = item.startDate
                      ? convertUTCToCalendarDate(item.startDate)
                      : undefined;
      ...
      if (h.key === "priority") {
        const priorityCfg = taskPriorityConfigs.find(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild className="w-full h-full">
              <button className="w-full h-full flex items-center justify-center rounded-xs text-foreground text-xs font-medium transition-opacity hover:bg-muted overflow-hidden px-3">
                <span
                  className={cn(
                    "truncate w-full text-center flex items-center justify-center",
                    !cycle && "text-muted-foreground font-normal",
                  )}
                >
                  {cycle ? (
        ...
        <div
          className={cn(
            ...
            displayOptions?.wrapText
              ? "truncate"
              : "whitespace-normal break-words line-clamp-2 text-center",
          )}
        >
          {item[h.key] || "—"}
        </div>
        ...
        >
          <table className="w-full border-collapse table-auto min-w-max text-xs relative">
            <thead className="sticky top-0 z-30 bg-card shadow-sm">
              <tr className="h-9 border-b">
                ...
                {headers.map((h) => (
                  <th
                    key={h.key}
                    className={cn(headerCellCls, "min-w-[150px]")}
                  >
                    {h.label}
                  </th>
                ))}
                <th
                  className="w-10 px-2 text-center align-middle bg-card sticky right-0 z-40 border-l"
            ...
            <tbody className="bg-background">
              {tasks.map((task) => {
                ...
                return (
                  <React.Fragment key={task.id}>
                    <tr className="border-b hover:bg-muted/30 group h-9 transition-colors relative">
                      <td className={cn(bodyCellCls, "px-2 border-r-0")}>
                        <Button
                          onClick={() => toggleTaskExpansion(task.id)}
                          className={cn(
                            "p-0 h-6 w-6 shrink-0",
                            taskSubtasks.length === 0 && "invisible",
                          )}
                          variant="ghost"
                          size="sm"
                      ...
                      {shouldShowField("id", "ID") && (
                        <td
                          className={cn(
                            bodyCellCls,
                            "px-4 text-center text-gray-500 font-medium w-[80px] min-w-[80px]",
                          )}
                        >
                      ...
                      {shouldShowField("task", "Task") && (
                        <td
                          className={cn(
                            bodyCellCls,
                            "min-w-[250px] px-4 text-left",
                            displayOptions?.wrapText ? "max-w-xs" : "",
                          )}
                        >
                      ...
                      <td
                        className="w-10 px-2 text-center sticky right-0 z-20 bg-card group-hover:bg-muted border-l"
                        style={{ boxShadow: "-2px 0 4px rgba(0,0,0,0.02)" }}
                      >
                        <DropdownMenu>
                    ...
                    {isExpanded &&
                      taskSubtasks.map((subtask) => (
                        <tr
                          key={subtask.id}
                          className="border-b hover:bg-muted/30 bg-muted/5 group h-9 transition-colors relative"
                        >
                          <td className={cn(bodyCellCls, "px-2 border-r-0")} />
                          <td className={cn(bodyCellCls, "px-2")}>
                            <Checkbox
                          ...
                          {shouldShowField("task", "Task") && (
                            <td
                              className={cn(
                                bodyCellCls,
                                "min-w-[250px] pl-10 pr-4 text-left",
                                displayOptions?.wrapText ? "max-w-xs" : "",
                              )}
                            >
                              <div className="flex items-center gap-2 overflow-hidden w-full group/taskname">
                    ...
                    {isExpanded && addingSubtaskToTask === task.id && (
                      <tr className="border-b h-9 bg-blue-50/20 group/add-subtask shadow-inner">
                        <td className={cn(bodyCellCls, "px-2 border-r-0")} />
                        <td className={cn(bodyCellCls, "px-2")}>
                        ...
                        <td
                          className={cn(
                            bodyCellCls,
                            "px-4 text-center text-xs text-blue-400 font-medium w-[80px] min-w-[80px]",
                          )}
                        >
                          Auto
                        </td>
                              ...
                              {(() => {
                                const currentType =
                                  taskTypes.find(
                                    (t) =>
                                      t.value ===
                                      (newSubtaskData.taskType || "task"),
                                  ) || null;
                                return currentType
                                  ? renderTaskTypeVisual(currentType, "h-4 w-4")
                                  : null;
                              })()}
                            </div>
                            <Input
                        ...
                        {headers.map((h) => {
                          const cellCls =
                            "flex items-center justify-center gap-2 w-full h-9 cursor-pointer hover:bg-muted/50 px-2 rounded transition-colors group/cell text-center";
                        ...
                        <td
                          className="w-10 px-2 border-l sticky right-0 z-20 bg-card"
              ...
              {!showAddTask ? (
                <tr
                  className="h-9 group cursor-pointer hover:bg-muted/10 transition-colors"
                  onMouseEnter={() => setIsAddTaskRowHovered(true)}
                  onMouseLeave={() => setIsAddTaskRowHovered(false)}
                >
                  <td
                    colSpan={1}
                    className={cn(bodyCellCls, "px-2 border-r-0")}
                  />
                  ...
                  {headers.map((h) => (
                    <td key={h.key} className={bodyCellCls} />
                  ))}
                  <td
                    className="sticky right-0 bg-card border-l w-10"
                    style={{ boxShadow: "-4px 0 8px rgba(0,0,0,0.05)" }}
                  />
                </tr>
              ) : (
                <tr className="h-9 border-b bg-blue-50/20">
                  <td className={cn(bodyCellCls, "px-2 border-r-0")} />
                  <td className={cn(bodyCellCls, "px-2")} />
                  <td
                    className={cn(
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <Input
                        autoFocus
                        placeholder={
                          (newTaskData.taskType || selectedAddTaskType) ===
                          "milestone"
                            ? "Milestone name..."
                            : "Task name..."
                        }
