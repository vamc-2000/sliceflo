...
import { getRelationshipIcon, getRelationshipLabel, getRelationshipIconColor } from '@/utils/relationship-utils';
...
const GanttConnectors = ({
    ...
    const connectors = useMemo(() => {
        ...
        return paths;
    }, [allTaskFeatures, gantt, timelineStartDate, getTaskRelationships, tasks, subtasks]);
                                                        ...
                                                        {sortFields.filter(f => !['id', 'task', 'taskType', 'status', 'cycle', 'assignee', 'startDate', 'endDate', 'priority'].includes(f.id)).length > 0 ? (
                                                            sortFields.filter(f => !['id', 'task', 'taskType', 'status', 'cycle', 'assignee', 'startDate', 'endDate', 'priority'].includes(f.id)).map((field) => (
                                                                <div
                                                                    ...
                                                                    className="grid grid-cols-[20px_1fr] items-center px-2 py-1 hover:bg-muted rounded cursor-pointer"
                                                                >
                                                                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${field.isSelected ? 'bg-primary border-primary' : 'border-input'}`}>
                                                                        {field.isSelected && <Check className="h-3 w-3 text-primary-foreground" />}
                                                                    </div>
                                                                    <span className="text-xs font-medium text-primary truncate">{field.fieldName}</span>
                                                                </div>
                                                            ))
                                                        ) : (
                                                            <p className="text-[10px] text-muted-foreground px-2 py-4 italic">No custom fields</p>
                                                        )}
                                                    </div>
                                                </div>
                                    ...
                                    <GanttFeatureList className='space-y-0'>
                                        {allTaskFeatures.map((feature) => (
                                            feature.ganttFeature ? (
                                                ...
                                                <div key={feature.id} style={{ height: 'var(--gantt-row-height)' }} className="border-b border-border/50" />
                                            )
                                        ))}
                                        {showConnectors && (
                                            <GanttConnectors
