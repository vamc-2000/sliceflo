...
export function GanttFieldVisibilityPopup({
  projectId,
  children,
}: GanttFieldVisibilityPopupProps) {
  ...
  const isFieldVisible = (fieldId: string, isSystem: boolean): boolean => {
    ...
    if (isSystem && systemFieldVisibility[legacyKey] !== undefined)
  ...
  const hiddenFields = allFields.filter(
    (f) => !isFieldVisible(f.id, f.isSystem),
  );
