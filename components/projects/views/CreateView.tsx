...
export function CreateView({ projectId }: CreateViewProps) {
  ...
  const renderIcon = () => {
    ...
    if (iconObj) {
      ...
      return (
        <IconComponent size={20} color={selectedIconData?.color || "#001F3F"} />
      );
    }
  ...
  const getValueOptionsForField = (fieldId: string): any[] => {
    ...
    if (
      customField &&
      (customField.type === "select-one" || customField.type === "select-many")
    ) {
  ...
  const renderValueInput = (filter: FilterCriteria, index: number) => {
    ...
    if (
      ["date-is-today", "date-is-this-week", "date-is-this-month"].includes(
        filter.condition,
      )
    ) {
    ...
    if (options.length > 0) {
      return (
        ...
        >
          <SelectTrigger
            className="h-10 bg-gray-50 border-gray-200 focus:ring-0"
            data-testid={`view-filter-value-${index}`}
          >
          ...
          <SelectContent className="border-0 border-b-[5px] border-primary">
            {options.map((opt) => (
              <SelectItem
                key={opt.value}
                value={opt.value}
                data-testid={`view-filter-value-${index}-option-${opt.value}`}
              >
        ...
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {filter.value ? (
                formatLocalDate(new Date(filter.value))
              ) : (
          ...
          >
            <CalendarPicker
              selectedDate={
                filter.value
                  ? convertUTCToCalendarDate(filter.value)
                  : undefined
              }
              onDateSelect={(date) =>
                updateFilter(filter.id, {
                  value: date ? convertSelectedDateToUTC(date) : "",
                })
              }
            />
          </PopoverContent>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg p-4 bg-muted/50">
              <div className="flex items-start gap-4">
                {/* Icon Section */}
                <div className="flex flex-col">
                  <label className="text-xs font-medium text-muted-foreground mb-2 h-4">
                    Icon
                  </label>
                  <button
                    type="button"
                ...
                {/* View Name */}
                <div className="w-80">
                  <label className="block text-xs font-medium text-muted-foreground mb-2 h-4">
                    View name
                  </label>
                  <Input
                    type="text"
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1 pr-6">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed mb-4">
                    Add a detailed description for this tailored view to help
                    your team understand its purpose.
                  </p>
              ...
              <div className="flex justify-between items-center">
                <div className="flex-1 pr-6">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Select the layout type (List, Board, etc.) in which you want
                ...
                <div className="flex-none">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={!viewType ? "text-muted-foreground" : ""}
                          >
                            {viewType
                              ? viewType === "list"
                                ? "List"
                    ...
                    <DropdownMenuContent
                      align="end"
                      className="w-[320px] border-0 border-b-[5px] border-primary"
                    >
                      <DropdownMenuItem
                        onClick={() => setViewType("list")}
                        className="gap-2 cursor-pointer"
                        data-testid="view-type-option-list"
                      >
                        <List className="h-4 w-4 text-muted-foreground" />
                        <span>List</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        ...
                        className="gap-2 cursor-pointer"
                        data-testid="view-type-option-kanban"
                      >
                        <SquareKanban className="h-4 w-4 text-muted-foreground" />
                        <span>Kanban</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        ...
                        className="gap-2 cursor-pointer"
                        data-testid="view-type-option-gantt"
                      >
                        <ChartGantt className="h-4 w-4 text-muted-foreground" />
                        <span>Gantt</span>
                      </DropdownMenuItem>
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <div
                  className={cn(
                    "flex-none",
                    viewType === "gantt" && "pointer-events-none",
                  )}
                >
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span
                            className={!groupBy ? "text-muted-foreground" : ""}
                          >
                            {groupBy
                              ? groupByOptions.find((o) => o.option === groupBy)
                                  ?.label || groupBy
                              : "Select field"}
                          </span>
                        </div>
            ...
            <div className="border border-border border-l-4 border-l-primary rounded-lg p-4 bg-card shadow">
              <div className="flex justify-between items-start mb-6">
                <div className="flex-1 pr-6">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Customize your view by applying specific task filters that
                    will be saved for this view.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {filters.length === 0 ? (
              ...
              <div className="space-y-4 mt-6">
                {filters.map((filter, index) => (
                  <div key={filter.id} className="flex items-center gap-3">
                    <div className="w-20">
                      {index === 0 ? (
                        ...
                        <Select
                          ...
                          onValueChange={(val) =>
                            updateFilter(filter.id, { operator: val as any })
                          }
                        >
                          <SelectTrigger
                            className="h-10 bg-muted/50 border-border focus:ring-0"
                            data-testid={`view-filter-operator-${index}`}
                          >
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent className="border-0 border-b-[5px] border-primary">
                            <SelectItem
                              value="AND"
                              data-testid={`view-filter-operator-${index}-option-AND`}
                            >
                              AND
                            </SelectItem>
                            <SelectItem
                              value="OR"
                              data-testid={`view-filter-operator-${index}-option-OR`}
                            >
                              OR
                            </SelectItem>
                          </SelectContent>
                    ...
                    >
                      <SelectTrigger
                        className="w-48 h-10 bg-muted/50 border-border"
                        data-testid={`view-filter-field-${index}`}
                      >
                        ...
                        {availableFields.map((f) => {
                          ...
                          return (
                            <SelectItem
                              ...
                              disabled={isGrouped}
                              className={cn(
                                isGrouped && "opacity-50 cursor-not-allowed",
                              )}
                    ...
                    >
                      <SelectTrigger
                        className="w-40 h-10 bg-muted/50 border-border"
                        data-testid={`view-filter-condition-${index}`}
                      >
                      ...
                      <SelectContent className="border-0 border-b-[5px] border-primary">
                        {getConditionsForField(filter.field).map((cond) => (
                          <SelectItem
                            key={cond}
                            value={cond}
                            data-testid={`view-filter-condition-${index}-option-${cond}`}
                          >
                ...
                disabled={
                  ...
                  filters.some(
                    (f) =>
                      !f.field ||
                      !f.condition ||
                      (f.condition !== "is-empty" &&
                        f.condition !== "is-not-empty" &&
                        !f.value),
                  )
                }
