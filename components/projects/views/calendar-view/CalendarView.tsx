...
const eventStyleGetter = (
  ...
  statusConfigs: Array<{ value: string; color: string }> = [],
) => {
  if (view === "day") {
    return {
      style: {
  ...
  const statusColor = task.status
    ? statusConfigs.find((s) => s.value === task.status)?.color
    : undefined;
  const priorityColor = task.priority
    ? priorityConfigs.find((p) => p.value === task.priority)?.color
    : undefined;
  const borderColor = statusColor || priorityColor || "#34C759";
  ...
  return (
    <div className="flex items-center justify-between gap-2 px-4 py-2 border-b border-border bg-card">
      ...
      <div className="flex items-center gap-1">
        ...
        {/* View Toggle Buttons */}
        <div className="h-8 flex items-center gap-1 bg-muted rounded p-1">
          <Button
...
const UnscheduledTasksPanel = ({
  ...
  return (
    <div className="w-80 border-l border-border bg-muted flex flex-col p-4 gap-2">
      {/* Header */}
      <div className="flex items-center justify-between">
      ...
      <div className="border border-border bg-card rounded-md">
        <div className="flex items-center justify-around text-xs text-muted-foreground border-b border-border p-2">
          <input
            type="checkbox"
              ...
              {tasks.map((task) => (
                ...
                >
                  ...
                  <GripVertical
                    ...
                    data-testid={`calendar-unscheduled-task-grip-${task.id}`}
                  />
...
const buildSpanRows = (
  ...
  const spanEvents: SpanEvent[] = activeEvents.map((event) => {
    ...
    if (eventStart < sprintStart) {
      ...
      startCol = sprintDates.findIndex(
  ...
  spanEvents.forEach((spanEvent) => {
    ...
    const rowIndex = rows.findIndex(
      (row) =>
        !row.some(
          (e) =>
            e.startCol <= spanEvent.endCol && e.endCol >= spanEvent.startCol,
        ),
    );
...
const SprintView = ({
  ...
  taskPriorityConfigs?: Array<{
    _id: string;
    value: string;
    label: string;
    color: string;
  }>;
  taskStatusConfigs?: Array<{ value: string; color: string }>;
}) => {
  ...
  const resizePreview = useRef<{ startCol: number; endCol: number } | null>(
    null,
  );
  ...
  const spanRows = useMemo(
    () => buildSpanRows(allEvents, sprintDates),
    [allEvents, sprintDates],
  );
  ...
  const handleResizeMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (resizingEvent.edge === "right") {
        newEndCol = Math.max(
          resizingEvent.originalStartCol,
          Math.min(13, resizingEvent.originalEndCol + deltaCols),
        );
      } else {
        newStartCol = Math.min(
          resizingEvent.originalEndCol,
          Math.max(0, resizingEvent.originalStartCol + deltaCols),
        );
      }
  ...
  const handleResizeMouseUp = useCallback(() => {
    if (resizingEvent && resizePreview.current) {
      ...
      if (newStartDate && newEndDate) {
        ...
        const subtask = !task
          ? subtasks.find((s) => s.id === resizingEvent.eventId)
          : null;
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-card border border-border">
      {/* Column Headers */}
      <div
        ...
        {sprintDates.map((currentDate) => {
          ...
          return (
            <div
              ...
              className={cn(
                "text-center py-3 border-r border-border last:border-r-0 px-1",
      ...
      <div className="flex-1 overflow-y-auto relative" ref={containerRef}>
          ...
          {sprintDates.map((currentDate) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "relative group",
                  dragOverDate === dateKey && "ring-2 ring-primary ring-inset",
                )}
                onDragOver={(e) => {
                  e.preventDefault();
                  ...
                  if (taskId) {
                    ...
                    if (item) {
                      ...
                      if (oldStart && oldEnd) {
                        ...
                        newEnd = convertSelectedDateToUTC(
              ...
              >
                ...
                {!isWeekendDay && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="absolute bottom-2 right-1 h-6 w-6 p-0 rounded bg-primary text-primary-foreground flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity hover:scale-110 hover:bg-primary"
                    style={{ zIndex: 10 }}
                    onClick={(e) => {
                      e.stopPropagation();
        ...
        >
          {spanRows.map((row, rowIndex) => (
            <div key={rowIndex} className="relative" style={{ height: "24px" }}>
              {row.map(({ event, startCol, endCol }) => {
                const task = event.resource.task;
                const statusColor = task.status
                  ? taskStatusConfigs?.find((s) => s.value === task.status)
                      ?.color
                  : undefined;
                const priorityColor = task.priority
                  ...
                  : undefined;
                const borderColor = statusColor || priorityColor || "#34C759";
                const cfg = taskPriorityConfigs?.find(
                ...
                return (
                  <div
                    ...
                    style={(() => {
                      ...
                      const displayStartCol = isResizing
                        ? resizePreviewState!.startCol
                        : startCol;
                      const displayEndCol = isResizing
                        ? resizePreviewState!.endCol
                        : endCol;
                  ...
                  >
                    <div
                      ...
                      className="group/event h-full text-xs px-2 cursor-pointer rounded border-l-[3px] bg-card hover:bg-muted shadow-sm flex items-center gap-1 overflow-visible relative"
                    ...
                    >
                      ...
                      <span className="font-medium text-foreground truncate px-2">
                        {event.title}
                      </span>
                      {/* {cfg && (
                                                <Badge
...
export function CalendarView({ projectId }: CalendarViewProps) {
    ...
    addTask(
      {
        ...
        endDate: taskData.endDate
          ...
          : convertSelectedDateToUTC(taskData.startDate),
  ...
  const handleEventDrop = ({ event, start, end }: any) => {
    ...
    const adjustedEnd = event.allDay ? new Date(newEnd.getTime() - 1) : newEnd;
  ...
  const handleSprintNavigate = useCallback(
  ...
  const events = useMemo(() => {
    ...
    const projectSubtasks = subtasks.filter(
      (st) => st.parentTaskId && projectTaskIds.includes(st.parentTaskId),
    );
    ...
    if (filters.assignees.length > 0) {
      filtered = filtered.filter(
        (event) =>
          ...
          filters.assignees.includes(event.resource.assignee),
      );
    }
    ...
    if (filters.statuses.length > 0) {
      filtered = filtered.filter(
        (event) =>
          ...
          filters.statuses.includes(event.resource.status),
      );
    }
    ...
    if (filters.priorities.length > 0) {
      filtered = filtered.filter(
        (event) =>
          ...
          filters.priorities.includes(event.resource.priority),
      );
    }
