...
export const CalendarEventHoverCard = ({
  task,
  position,
  isSubtask = false,
  parentTaskId,
  ...
  const { projects, getTaskStatusConfigs, getTaskPriorityConfigs } =
    useProjectsStore();
  ...
  const { tasks, subtasks, getTaskRelationships, updateTask, updateSubtask } =
    useTasksStore();
  ...
  const statusOption = task.status
    ? taskStatusConfigs.find(
        (c) => c.value === task.status || c.label === task.status,
      )
    : null;
  ...
  const borderColor = statusOption?.color || priorityOption?.color || "#34C759";
    ...
    >
      ...
      >
            ...
            <Popover open={isPriorityOpen} onOpenChange={setIsPriorityOpen}>
              <PopoverTrigger asChild>
                <div
                  ...
                  data-testid={`calendar-event-hovercard-priority-trigger-${task.id}`}
                >
                  {priorityOption ? (
                    <Badge
                      variant="secondary"
                    ...
                    <Badge
                      variant="outline"
                      className="text-xs px-1 h-6 gap-1 text-muted-foreground bg-muted border-none rounded-full"
                    >
                      <Flag className="h-4 w-4" />
                    </Badge>
                  )}
        ...
        <h3
          ...
          data-testid={`calendar-event-hovercard-name-text-${task.id}`}
        >
          {task.name}
        </h3>
        ...
        <div className="flex items-center gap-2">
          ...
          {task.startDate && (
            <Badge
              variant="secondary"
              className="text-xs font-normal h-6 px-2 py-0.5 flex items-center gap-1 bg-muted text-muted-foreground border-none"
            >
          ...
          {task.endDate ? (
            <Badge
              ...
              className={cn(
                ...
                getDaysRemaining(task.endDate) !== null &&
                  getDaysRemaining(task.endDate)! < 0
            ...
            !task.startDate && (
              <Badge
                variant="outline"
                className="text-xs gap-1 px-1.5 h-6 text-muted-foreground bg-muted border-none rounded-full"
              >
