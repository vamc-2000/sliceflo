...
import { ArrowLeft, Share2, ChevronUp, ChevronDown } from "lucide-react";
...
export default function StatusHistoryModal({
  ...
  return (
    <Sheet open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-[550px] gap-0 top-4 right-0 bottom-4 h-[calc(100vh-2rem)] p-0 flex flex-col overflow-hidden bg-background rounded-l-xl border shadow-2xl transition-all duration-300 [&>button]:hidden"
      >
        {/* Top Header Row */}
            ...
            {history.length === 0 ? (
              ...
              history.map((entry) => {
                ...
                const member = workspaceMembers.find(
                  (m) =>
                    m.userId === entry.changedBy ||
                    m.id === entry.changedBy ||
                    m._id === entry.changedBy,
                );
                ...
                const avatarUrl = member?.profilePicture
                  ...
                  : undefined;
                const displayName =
                  member?.name || entry.changedBy || "Unknown";
