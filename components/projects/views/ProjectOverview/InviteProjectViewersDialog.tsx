...
export default function InviteProjectViewersDialog({
  ...
  const { workspaceMembers, fetchWorkspaceMembers, currentWorkspace } =
    useWorkspaceStore();
  ...
  const canAdd =
    selected.length > 0 || rows.some((r) => r.email && isValidEmail(r.email));
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const validEmails = rows
        .filter((r) => r.email.trim() && isValidEmail(r.email))
      ...
      if (validEmails.length > 0) {
        ...
        const invited = fresh.filter((u) =>
          validEmails.includes(u.email.toLowerCase()),
        );
        if (invited.length > 0) {
          await addViewersToProject(
            projectId,
            invited.map((u) => String(u.userId)),
          );
        }
      }
          ...
          <div className="w-1/2 p-6 flex flex-col overflow-y-auto">
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center border border-input rounded-lg px-3 h-9 bg-muted">
                ...
                <Button
                  ...
                  className="h-7 w-7 hover:bg-muted/80 rounded"
                >
            ...
            <div className="space-y-3 mt-2">
              ...
              {rows.map((row, index) => (
                <div key={index} className="w-full space-y-1">
                  <div className="flex items-center gap-2 w-full">
                    <div
                      className={`flex items-center flex-1 p-1 rounded-md bg-background border ${row.email && !isValidEmail(row.email) ? "border-red-500" : "border-input"}`}
                    >
                      <Input
              ...
              <div className="flex justify-end">
                <span
                  className="text-xs text-primary-text cursor-pointer hover:underline"
                  onClick={() => setRows([...rows, { email: "" }])}
                >
                  + Add more
                </span>
              </div>
            ...
            <div className="flex justify-center mt-6">
              <Button
                ...
                className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-md px-8 py-1.5"
              >
