...
const ViewAllMembersModal: React.FC<ViewAllMembersModalProps> = ({
  ...
  const handleConfirmRemove = async () => {
    ...
    try {
      ...
      console.error(
        `Failed to remove ${type === "members" ? "member" : "viewer"}:`,
        error,
      );
    } finally {
      setIsRemoving(false);
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="!max-w-2xl !w-full border-0 border-b-[5px] border-b-primary rounded-lg bg-card">
          <DialogHeader className="flex flex-row items-center justify-between">
            <DialogTitle className="text-primary-text text-sm font-bold">
              {type === "members"
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b border-border bg-muted">
                  <TableHead className="border-r border-border text-center text-primary-text text-xs">
                    {type === "members" ? "Members" : "Viewers"}
                  </TableHead>
                  <TableHead className="text-center text-primary-text text-xs">
                    Action
                  </TableHead>
                </TableRow>
              </TableHeader>
                  ...
                  >
                    ...
                    <TableCell className="text-center">
                      <button
                        ...
                        className={`bg-muted text-xs hover:cursor-pointer hover:underline px-3 py-1 rounded-md ${
                          (type === "members" && member.role === "owner") ||
                          (type === "members" &&
                            member.id === currentUserId &&
                            leaderIds.includes(member.id))
