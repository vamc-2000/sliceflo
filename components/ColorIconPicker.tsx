...
const ColorIconPicker: React.FC<ColorIconPickerProps> = ({
  ...
  const handleDeleteImage = async (): Promise<void> => {
    try {
      ...
      if (uploadedImageId && onDelete) {
        console.log(
          "🗑️ Requesting parent to delete image, ID:",
          uploadedImageId,
        );
  ...
  const handleUploadNew = async (): Promise<void> => {
    try {
      ...
      if (uploadedImageId && onDelete) {
        console.log(
          "🗑️ Deleting old image before new upload, ID:",
          uploadedImageId,
        );
  ...
  const content = (
    <div
      className="bg-card text-card-foreground rounded-lg shadow-xl w-xs flex flex-col overflow-hidden border border-border border-b-4 border-b-primary"
      data-testid="modal-content"
      onClick={isInline ? undefined : (e) => e.stopPropagation()}
    >
      {/* Header */}
      <div
        className="flex justify-between items-center px-2 border-b border-border"
      ...
      >
        ...
        {/* Color Palette */}
        <div className="flex flex-wrap gap-1" data-testid="color-palette">
          {/* No Color Button */}
          <button
            ...
            className={`w-5 h-5 rounded-full border-2 flex items-center justify-center relative ${
              selectedColor === "transparent" ? "border-ring" : "border-border"
            }`}
          ...
          {colors.map((color) => (
            <button
              ...
              className={`w-5 h-5 rounded-full border-2 ${
                selectedColor === color
                  ? "border-ring ring-1 ring-primary"
                  : "border-border"
              }`}
      ...
      {mode === "icon" && (
        ...
        >
          {activeTab === "Icon" && (
            <div className="p-3" data-testid="icon-tab-content">
              {/* Search */}
              <div
  ...
  "
          ...
          {activeTab === "Upload" && (
            ...
            >
              ...
              {!uploadedImage ? (
                <div
                  className="flex-1 flex flex-col items-center justify-center bg-muted py-4 rounded-md"
                ...
                >
                  ...
                  <div className="flex gap-2 mb-3" data-testid="upload-actions">
                    <button
