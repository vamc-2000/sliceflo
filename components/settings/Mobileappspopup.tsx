...
const MobileAppPopup: React.FC<Props> = ({ open, onClose }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center w-full bg-black/50">
      <div className="bg-card text-foreground rounded-xl shadow-lg w-full max-w-4xl p-6 relative mx-4 border-b-4 border-brand">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-muted-foreground hover:text-brand transition-colors"
          aria-label="Close"
        >
          <X className="w-4 h-4" />
        </button>

        <h2 className="text-xl font-semibold mb-6 text-brand">
          Download App
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <div className="text-center order-1 md:order-2">
            ...
            <div className="my-4 text-sm font-semibold text-[var(--muted-foreground)]">
              OR
            </div>
